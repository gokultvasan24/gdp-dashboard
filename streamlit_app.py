...
# ============================================================
# 3️⃣ STOCK ANALYTICS (1H – 1 YEAR DATA)
# ============================================================

with tabs[2]:

    st.header("Stock-Level Analytics (1 Hour – 1 Year)")

    stock = st.selectbox("Select Stock", ALL_STOCKS)

    intraday = download_data(stock, period="1y", interval="1h")
    ...
    if not intraday.empty:
        ...
        # Remove weekends
        intraday = intraday[intraday.index.dayofweek < 5]
        ...
        col4.metric("Range %", f"{((close.iloc[-1]-low_52w)/(high_52w-low_52w))*100:.2f}%")

        col5, col6 = st.columns(2)
        col5.metric("RSI (14)", f"{rsi.iloc[-1]:.2f}")
        col6.metric("MACD", f"{macd.iloc[-1]:.2f}")

        st.subheader("1H Price Chart (52 Week Range Projection)")

        fig = px.line(
            intraday,
            x=intraday.index,
            y="Close",
            title=f"{stock} - 1H Price (1 Year)"
        )
        ...
        st.warning("No 1H data available.")
